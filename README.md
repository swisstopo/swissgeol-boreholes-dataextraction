...
- If you are using the classifier `bert` with a trained local [model](#train-bert-model), specify its folder path using the `-p` or `--model-path` flag. The folder has to contain all files generated when saving a model checkpoint with the transformers librairy. Such models can be downloaded from the s3 bucket `stijnvermeeren-boreholes-models`. Currently there are two models there, with the one called `best_model_class_balancing` performing the best.
