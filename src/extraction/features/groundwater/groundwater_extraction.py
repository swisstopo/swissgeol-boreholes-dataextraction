...
class GroundwatersInBorehole:
    ...
    def merge_compatible_candidates(self, terrain_elevation: float | None):
        ...
        pages: dict[int, list[FeatureOnPage[Groundwater]]] = {}
        for gw in self.groundwater_feature_list:
            pages.setdefault(gw.page_number, []).append(gw)
...
# the list of already seen depths to avoid confusion with stratigraphy.
...
#         def mutually_compatible(group: list[FeatureOnPage[Groundwater]]) -> bool:
#             """Returns True if no two members of the group conflict with each other."""
#             return all(not conflicts(group[i], group[j]) for i in
# range(len(group)) for j in range(i + 1, len(group)))
