...
def merge_parallel_lines_quadtree_clean(lines: list[Line], tol: int, angle_threshold: float) -> list[Line]:
    """Merge parallel lines that are close to each other.
    ...
    # Compute bounds for the quadtree
    max_x = max(line.end.x for line in lines)
    max_y = max(max(line.start.y, line.end.y) for line in lines)

    quadtree = LinesQuadTree(max_x, max_y)
    keys_queue = queue.Queue()

    # Populate the quadtree
    for line in lines:
        key = quadtree.add(line)
        keys_queue.put(key)

    # Merge loop
    while not keys_queue.empty():
        current_key = keys_queue.get()

        if current_key not in quadtree.hashmap:
            continue

        current_line = quadtree.hashmap[current_key]
        neighbours = quadtree.neighbouring_lines(current_key, tol)

        for neighbour_key, neighbour_line in neighbours.items():
            if not _are_parallel(current_line, neighbour_line, angle_threshold):
                continue
            if not _are_close(current_line, neighbour_line, tol):
                continue

            merged_line = _merge_lines(current_line, neighbour_line)
            if merged_line:
                quadtree.remove(current_key)
                quadtree.remove(neighbour_key)
                new_key = quadtree.add(merged_line)
                keys_queue.put(new_key)
                break  # Exit early to restart with the updated merged line

    return list(quadtree.hashmap.values())
