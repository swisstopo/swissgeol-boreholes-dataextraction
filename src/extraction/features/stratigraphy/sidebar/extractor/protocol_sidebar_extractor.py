...
class ProtocolSidebarExtractor:
    ...
    def find_in_words(
        ...
        processed_sidebars = [
            ...
            if ProtocolSidebarExtractor._is_below_header(sidebar, lines, header_keywords, max_header_gap)
            and ProtocolSidebarExtractor._is_table_like(sidebar, lines)
            for processed_sidebar in sidebar.process()
            if processed_sidebar.is_valid()
        ]
    ...
    def _is_below_header(
        ...
        Args:
            sidebar (ProtocolSidebar): The candidate sidebar.
            lines (list[TextLine]): All text lines in the page.
            header_keywords (tuple[str, ...]): Allowed header keywords.
            max_header_gap (float): Maximum vertical distance between header and first entry.

        Returns:
            bool: True if a matching header is found above the sidebar, False otherwise.
        """
    ...
    def _is_table_like(sidebar: ProtocolSidebar, lines: list[TextLine]) -> bool:
        ...
        The heuristic is intentionally lightweight: for at least half of the entries,
        there must be a non-empty text line to the right on approximately the same row.
        ...
        Returns:
            bool: True if the sidebar appears in a table-like structure, False otherwise.
        """
        matching_rows = 0
        ...
        return matching_rows >= max(1, len(sidebar.entries) // 2)
