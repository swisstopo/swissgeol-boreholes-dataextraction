...
class MaterialDescriptionRectWithSidebarExtractor:
    ...
    # def process_page(self) -> list[ExtractedBorehole]:
    #     """Process a single page and extract boreholes using a TRUE OR at pair level.

    #     Behavior when enabled:
    #     - Always keep all sidebar/depth-based pairs (include_descriptions_without_sidebar=False).
    #     - Additionally keep description-only pairs ONLY if table structures are present.
    #     - The final set of pairs is the UNION (superset) of both sources, so depth pairs are never dropped
    #         because table-mode candidates exist.

    #     Controlled by:
    #     - matching_params["require_table_or_depth_union"] (bool): enable this behavior.
    #     """
    #     enforce_union_or = self.matching_params.get("require_table_or_depth_union", False)

    #     if not enforce_union_or:
    #         # Default / legacy behavior
    #         filtered_pairs = self._extract_filtered_sidebar_pairs(include_descriptions_without_sidebar=True)
    #         boreholes = [self._create_borehole_from_pair(pair) for pair in filtered_pairs]
    #         return [
    #             b for b in boreholes
    #             if len(b.predictions) >= self.matching_params["min_num_layers"]
    #         ]

    #     has_table = bool(self.table_structures)

    #     # 1) Depth-based candidates (sidebar present): always compute + always include
    #     depth_pairs = self._extract_filtered_sidebar_pairs(include_descriptions_without_sidebar=False)

    #     # 2) Table-enabled candidates (may include no-sidebar), ONLY if table exists
    #     table_pairs: list[MaterialDescriptionRectWithSidebar] = []
    #     if has_table:
    #         table_pairs = self._extract_filtered_sidebar_pairs(include_descriptions_without_sidebar=True)

    #     # 3) TRUE UNION at pair level (keep all depth pairs, then add the remaining table pairs)
    #     # Use a stable key because Rect/objects may not be hashable and "p not in depth_pairs"
    #     # depends on object identity.
    #     def pair_key(p: MaterialDescriptionRectWithSidebar) -> tuple:
    #         sidebar_kind = getattr(p.sidebar, "kind", None) if p.sidebar else None
    #         sidebar_rect = tuple(p.sidebar.rect) if p.sidebar else None  # (x0,y0,x1,y1)
    #         mat_rect = tuple(p.material_description_rect)  # (x0,y0,x1,y1)
    #         # page_number is useful to disambiguate if keys are compared across pages
    #         return (self.page_number, sidebar_kind, sidebar_rect, mat_rect)

    #     seen = set()
    #     union_pairs: list[MaterialDescriptionRectWithSidebar] = []

    #     for p in depth_pairs:
    #         k = pair_key(p)
    #         if k not in seen:
    #             seen.add(k)
    #             union_pairs.append(p)
    ...
    #     # 4) Create boreholes and apply existing min layer filter
    #     boreholes = [self._create_borehole_from_pair(pair) for pair in union_pairs]

    #     logger.debug(
    #         "Page %s: union_or enabled | tables=%d |
    #              depth_pairs=%d | table_pairs=%d | union_pairs=%d | boreholes=%d",
    #         self.page_number,
    #         len(self.table_structures),
    #         len(depth_pairs),
    #         len(table_pairs),
    ...
    # #### table OR depth ####
    # def process_page(self) -> list[ExtractedBorehole]:
    #     """Create boreholes only if a page has (depth/sidebar) OR (table)."""
    #     enforce_or_gate = self.matching_params.get("require_table_or_depth", False)

    #     has_table = bool(self.table_structures)

    #     # 1) First: try depth-based extraction only (sidebar present)
    #     sidebar_pairs = self._extract_filtered_sidebar_pairs(include_descriptions_without_sidebar=False)

    #     if enforce_or_gate:
    #         # If we have depth, we're good (depth satisfies OR).
    #         if sidebar_pairs:
    #             filtered_pairs = sidebar_pairs
    #         else:
    #             # No depth found: only allow description-only fallback if a table exists
    #             if not has_table:
    #                 return []
    #             filtered_pairs = self._extract_filtered_sidebar_pairs(include_descriptions_without_sidebar=True)
    #     else:
