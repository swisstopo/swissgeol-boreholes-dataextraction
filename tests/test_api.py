...
def test_extract_data_valid_text_request(monkeypatch):
...
def test_extract_data_invalid_bbox_request(monkeypatch):
...
def test_extract_data_missing_file(monkeypatch):
    """Test the extract_data endpoint with a missing PDF file."""

    # Mock the load_pdf_from_aws to raise an exception
    def mock_load_pdf_from_aws(filename: str):
        raise HTTPException(status_code=404, detail="PDF document not found in S3 bucket.")

    monkeypatch.setattr(
        "your_module.load_pdf_from_aws", mock_load_pdf_from_aws
