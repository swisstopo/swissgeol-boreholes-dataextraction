...
def test_line_anchor():  # noqa: D103
    ...
    assert LayerDepths(start, end).line_anchor == fitz.Point(1, 1.5), (
    ...
    assert LayerDepths(start, end=None).line_anchor == fitz.Point(
        1, 1
    ), "When there is no end depth, the line anchor should be the bottom-right of the start depth."

    assert LayerDepths(start=None, end=end).line_anchor == fitz.Point(
        1, 2
    ), "When there is no start depth, the line anchor should be the top-right of the end depth."

    left = LayerDepthsEntry(5, fitz.Rect(0, 0, 1, 1))
    right = LayerDepthsEntry(10, fitz.Rect(2, 0, 3, 1))
    assert LayerDepths(left, right).line_anchor == fitz.Point(3, 0.5), (
...
def test_background_rect():  # noqa: D103
    ...
    assert (
        LayerDepths(start, end).background_rect is None
